...
namespace Stride.Assets.DataAssets
{
    ...
    internal class MaterialAssetCompiler : AssetCompilerBase
    {
        ...
        {
            yield return new BuildDependencyInfo(typeof(DataAsset), typeof(AssetCompilationContext), BuildDependencyType.CompileAsset);
        }

        protected override void Prepare(AssetCompilerContext context, AssetItem assetItem, string targetUrlInStorage, AssetCompilerResult result)
        {
            var asset = (DataAsset)assetItem.Asset;
            result.BuildSteps = new AssetBuildStep(assetItem);
            result.BuildSteps.Add(new MaterialCompileCommand(targetUrlInStorage, assetItem, asset, context));
        }

        private class MaterialCompileCommand : AssetCommand<DataAsset>
        {
            ...
            public MaterialCompileCommand(string url, AssetItem assetItem, DataAsset value, AssetCompilerContext context)
            ...
            protected override void ComputeParameterHash(BinarySerializationWriter writer)
            {
                ...
                foreach (var compileTimeDependency in ((DataAsset)assetItem.Asset).FindMaterialReferences())
                {
